...
开机启动分为两条独立链路：`NetSplitService` 使用延迟自动启动，托盘通过当前用户的登录任务启动并保持普通权限。托盘任务会在登录后等待约 15 秒，错过登录触发或短暂崩溃时由任务计划程序补启动并重试；托盘默认只显示通知区域图标，不会自动打开主窗口。首次安装保持关闭；升级会恢复升级前已经开启的分流状态，不会把原本关闭的状态自动打开。
